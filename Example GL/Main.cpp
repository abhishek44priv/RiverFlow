...
class  Example2 :public RfImguiWindow
{
	class RfGLDrawListRender
	{
		unsigned int VBO, VAO, EBO, countTriangles;
	...
	public:
		...
		void Draw(float mat[9])
		{
			shader.use();
			glBindVertexArray(VAO);
			shader.setMat3("proj", mat);
			glDrawElements(GL_TRIANGLES, countTriangles, GL_UNSIGNED_INT, 0);
		}

	};
	float rad = 0,step=0.01;
	RfDrawList drawlist;
	RfGLDrawListRender drawListRender;
...
protected:
	...
	void OnGLDraw() override
	{
		...
		//render
		{
			...
			auto projection = RfMat33_2::Projection_((float)GetWidth(), (float)GetHeight());
			auto cameraMat = RfMat33_2::Identity_();
			auto modelMat = RfMat33_2::Identity_();
			auto transform = RfMat33_2::Multiply_(projection, RfMat33_2::Multiply_(cameraMat, modelMat));
			drawListRender.Draw(transform.dst);
		}
...
#define ONLY_GUIpp
#ifdef ONLY_GUI
int WINAPI wWinMain(void* hInstance, void* hPrevInstance, wchar_t* pCmdLine, int nCmdShow)
#else
int main()
#endif // ONLY_GUI
{
